    ...
    describe('프로퍼티', () => {
        describe('validation', () => {
            ...
            describe('email', () => {
                ...
                describe('format', () => {
                    describe('이메일 주소 형식이 아닌 값', () => {
                        it('validation.email.format이 invalid임', () => {
                            loginForm.setData({
                                email: 'foobar'
                            })
                    ...
                    describe('이메일 주소 형식인 값', () => {
                        it('validation.email.format이 valid임', () => {
                            loginForm.setData({
                                email:'foo@domail.com'
                            })
            ...
            describe('password', () => {
                describe('required', () => {
                    describe('아무것도 입력하지 않음', () => {
                        it('validation.password.required이 invalid임', () => {
                            loginForm.setData({
                                password:''
                            })
                    ...
                    describe('입력 내용 있음', () => {
                        it('validation.password.required이 valid임', () => {
                            loginForm.setData({
                                password:'xxxx'
                            })
        ...
        describe('onlogin', () => {
            ...
            describe('solve', () => {
                it('solve 됨', done => {
                    ...
                    expect(loginForm.vm.error).to.equal('') //오류 메세지 초기화
                    ...
                    loginForm.vm.$nextTick(() => {
                        ...
                        loginForm.vm.$nextTick(() => { //resolve 에서 상태 반영
                            expect(loginForm.vm.error).to.equal('') //오류 메세지는 초기화된 그대로
            ...
            describe('reject', () => {
                it('reject 됨', done => {
                    ...
                    //퀵 이벤트
                    loginForm.find('button').trigger('click')
                    expect(onloginStub.called).to.equal(false) //아직 reject되지 않음
                    expect(loginForm.vm.error).to.equal('') //오류 메세지 초기화
                    expect(loginForm.vm.disableLoginAction).to.equal(true) // 로그인 액션 불가
                    ...
                    loginForm.vm.$nextTick(() => {
                        ...
                        expect(authInfo.password).to.equal(loginForm.vm.password)
                        loginForm.vm.$nextTick(() => {
                            expect(loginForm.vm.error).to.equal('login error!') //오류 메세지가 설정됨
                            expect(loginForm.vm.disableLoginAction).to.equal(false) //로그인 액션 가능
                            done()
                        })
                    })
